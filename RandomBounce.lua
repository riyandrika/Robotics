...
function sysCall_init()
    ...
    -- To calibrate
    motorAnglePerMetre = 23.8
    motorAnglePerRadian = 2.8
...
function sysCall_actuation() 
    ...
    if (stepCompletedFlag == true or stepCounter == 0) then
        ...
        elseif (newStepType == "turn_random") then
            ...
            random_angle_to_turn = math.random(90,180)
            
            motorAngleTargetL = posL - random_angle_to_turn * motorAnglePerRadian
            motorAngleTargetR = posR + random_angle_to_turn * motorAnglePerRadian
    ...
    if (stepType == "forward_until_bump") then  
        ...
        if (noisyDistance < 0.8 and noisyDistance > 0.00001) then -- more than a small value so that it does not go to next step when sensor is 0
